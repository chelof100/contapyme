-- ===== CONVERSIÓN A SINGLE TENANT - VERSIÓN CORREGIDA =====
...
-- Políticas para tablas de detalles nuevas (solo si existen)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'detalles_factura') THEN
    EXECUTE 'CREATE POLICY "authenticated_users_all_access" ON detalles_factura FOR ALL USING (auth.role() = ''authenticated'')';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'detalles_orden_compra') THEN
    EXECUTE 'CREATE POLICY "authenticated_users_all_access" ON detalles_orden_compra FOR ALL USING (auth.role() = ''authenticated'')';
  END IF;
  
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'transacciones_financieras') THEN
    EXECUTE 'CREATE POLICY "authenticated_users_all_access" ON transacciones_financieras FOR ALL USING (auth.role() = ''authenticated'')';
  END IF;
END $$;
...
-- Crear índices optimizados para single tenant (basados en columnas reales)
