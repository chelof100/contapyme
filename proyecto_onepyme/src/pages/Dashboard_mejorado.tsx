    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.facturas?.emitidas || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {dashboardData.facturas?.pendientes || 0} pendientes
              </p>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.ordenes?.abiertas || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {dashboardData.ordenes?.cerradas || 0} cerradas
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.productos?.total || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {dashboardData.productos?.stockBajo || 0} con stock bajo
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{dashboardData.pagos?.total || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                ${(dashboardData.pagos?.monto || 0).toLocaleString('es-AR')}
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{crmData.clientes?.activos || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {crmData.clientes?.prospectos || 0} prospectos
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{crmData.oportunidades?.abiertas || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                ${(crmData.oportunidades?.valorTotal || 0).toLocaleString('es-AR')}
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{crmData.actividades?.pendientes || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {crmData.actividades?.vencidas || 0} vencidas
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              ${(erpData.finanzas?.cashFlowMes || 0).toLocaleString('es-AR')}
            </div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {(erpData.finanzas?.presupuestoVsReal || 0).toFixed(1)}% vs presupuesto
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{erpData.empleados?.activos || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {erpData.empleados?.enLicencia || 0} en licencia
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{erpData.proyectos?.activos || 0}</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {(erpData.proyectos?.facturacionPendiente || 0).toFixed(1)} hs por facturar
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
        ...
        >
          ...
          <CardContent>
            <div className="text-2xl font-bold">{(erpData.finanzas?.rentabilidad || 0).toFixed(1)}%</div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">
                {(erpData.proyectos?.rentabilidadPromedio || 0).toFixed(1)}% proyectos
              </p>
              <ArrowRight className="h-4 w-4 text-muted-foreground" />
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {(dashboardData.productos?.stockBajo || 0) > 0 && (
              <div className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                <div className="flex items-center gap-2">
                  <Package className="h-4 w-4 text-red-600" />
                  <span className="text-sm font-medium">Stock Bajo</span>
                </div>
                <Badge variant="destructive">{dashboardData.productos?.stockBajo || 0}</Badge>
              </div>
            )}
            
            {(crmData.actividades?.vencidas || 0) > 0 && (
              <div className="flex items-center justify-between p-3 bg-orange-50 rounded-lg">
                <div className="flex items-center gap-2">
                  <Clock className="h-4 w-4 text-orange-600" />
                  <span className="text-sm font-medium">Actividades Vencidas</span>
                </div>
                <Badge variant="secondary">{crmData.actividades?.vencidas || 0}</Badge>
              </div>
            )}
            
            {(dashboardData.facturas?.pendientes || 0) > 0 && (
              <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                <div className="flex items-center gap-2">
                  <FileText className="h-4 w-4 text-yellow-600" />
                  <span className="text-sm font-medium">Facturas Pendientes</span>
                </div>
                <Badge variant="outline">{dashboardData.facturas?.pendientes || 0}</Badge>
              </div>
            )}
            
            {(erpData.empleados?.enLicencia || 0) > 0 && (
              <div className="flex items-center justify-between p-3 bg-blue-50 rounded-lg">
                <div className="flex items-center gap-2">
                  <User className="h-4 w-4 text-blue-600" />
                  <span className="text-sm font-medium">Empleados en Licencia</span>
                </div>
                <Badge variant="secondary">{erpData.empleados?.enLicencia || 0}</Badge>
              </div>
            )}
